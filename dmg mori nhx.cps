/**
  ...
  DMG Mori NHX post processor configuration.
...
properties = {
  ...
  useSmoothing: {
    ...
    values     : [
      ...
      {title:"Standard", id:"1"},
      {title:"High Speed", id:"3"},
      {title:"High Accuracy", id:"4"},
    ],
    value: "-1",
    scope: "post"
  },
  ...
  useG54x4: {
    title      : "Use G54.4",
    description: "Supports G54.4 for workpiece error compensation.",
    group      : "probing",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  makeBAxisOtherWay: {
    title      : "Reverse direction of B-axis",
    description: "Positive value moves B-axis in CCLW direction.",
    group      : "configuration",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  gotContinuousBAxis: {
    title      : "B-axis is continuous",
    description: "B-axis is contouring axis with no physical limits.  'No' defines an indexing table with 1 degree increments.",
    group      : "configuration",
    type       : "boolean",
    value      : true,
    scope      : "post"
...
function defineMachine() {
  if (!receivedMachineConfiguration) {
    if (true) {
      var useTCP = false;
      var bAxis;
      if (getProperty("gotContinuousBAxis")) {
        bAxis = createAxis({coordinate:1, table:true, axis:[0, (getProperty("makeBAxisOtherWay") ? -1 : 1) * -1, 0], cyclic:true, preference:1, tcp:useTCP});
      } else {
        bAxis = createAxis({coordinate:1, table:true, axis:[0, (getProperty("makeBAxisOtherWay") ? -1 : 1) * -1, 0], range:[0, 359.999], tcp:useTCP});
        abcFormat.setNumberOfDecimals(0);
    ...
    if (machineConfiguration.isMultiAxisConfiguration()) {
      machineConfiguration.setMultiAxisFeedrate(
        ...
        9999.9, // maximum output value for inverse time feed rates
...
function onOpen() {
  ...
  useSmoothing = getProperty("useSmoothing");
  useSmoothingType = getProperty("useSmoothingType");
...
function setSmoothing(mode) {
  ...
  if (mode) { // enable smoothing
    writeBlock(gFormat.format(332), "R" + smoothing.level + ".");
    if (getProperty("useSmoothingType") == "II") {
      writeBlock(gFormat.format(5), "P10000");
    } else {
...
function onParameter(name, value) {
  if (name == "action") {
    ...
    if (sText2[0] == "SMOOTH") {
      ...
      if (num.length == 2) {
        if ((num[1] < 0) || (num[1] > 4)) {
          error(localize("SMOOTH accuracy must be 0-4" + ": " + value));
          return;
        }
        rSmoothing = num[1];
      }
      if (num[0] == 0) {
        setProperty("useSmoothing", "-1");
      } else {
        setProperty("useSmoothingType", num[0] == 1 ? "I" : "II");
        setProperty("useSmoothing", num[1] == 0 ? "-9999" : num[1].toString());
      }
    } else {
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane || smoothing.cancel) {
    ...
    if (((insertToolCall || smoothing.cancel) ||
        ((newWorkOffset || newWorkPlane) && !getProperty("gotContinuousBAxis"))) &&
        !isFirstSection()) {
...
function onSectionEnd() {
  ...
  forceAny();
  setProperty("useSmoothing", useSmoothing);
  setProperty("useSmoothingType", useSmoothingType);
}
...
function onClose() {
  ...
  if (getProperty("palletChangeCode") != "none") {
    writeBlock(mFormat.format(parseFloat(getProperty("palletChangeCode")))); // call up RPP pallet system
  }
